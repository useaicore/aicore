...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError('');
    try {
      await signUp.email({ email, password, name, callbackURL: '/overview' }, {
    ...
    >
      ...
      <div className="mb-8">
        ...
        >
          ...
          <span className="text-[10px] font-bold uppercase tracking-widest" style={{ color: 'var(--success)' }}>
        ...
        <p className="text-[var(--text-muted)] text-sm">
          ...
          <Link href="/auth/login" className="text-[var(--gold-mid)] hover:text-[var(--gold-bright)] font-semibold transition-colors">
            Sign in
          </Link>
        </p>
      </div>
      ...
      {magicSent && (
        <div
          className="mb-5 px-4 py-3 rounded-[var(--radius-md)] text-sm"
          style={{ background: 'rgba(74,143,170,0.08)', border: '1px solid rgba(74,143,170,0.2)', color: 'var(--sky-bright)' }}
        >
          ✓ Check your inbox — your sign-up link is on its way.
        </div>
      )}
      ...
      <p className="text-[var(--text-faint)] text-[11px] text-center mt-5 leading-relaxed">
        By signing up you agree to our{' '}
        <Link href="#" className="underline hover:text-[var(--text-muted)] transition-colors">Terms</Link>
        {' '}and{' '}
        <Link href="#" className="underline hover:text-[var(--text-muted)] transition-colors">Privacy Policy</Link>.
      </p>
    </div>
  );
}
